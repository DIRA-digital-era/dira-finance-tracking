...
export default async function RequestsPage() {
   ...
   const pendingRequests = res.rows;
   ...
   return (
      <div className="space-y-6">
         ...
         <div className="flex gap-6 mb-8">
            <div className="bg-[var(--color-card)] rounded-xl p-6 border border-[var(--color-border)] w-1/2">
               <p className="text-[10px] text-gray-400 tracking-widest uppercase">Total Pending Amount</p>
               <p className="text-3xl font-mono text-[var(--color-foreground)] tracking-widest mt-2">{totalAmount.toLocaleString('en-US', {minimumFractionDigits: 0, maximumFractionDigits: 0})} XAF</p>
            </div>
            <div className="bg-[var(--color-card)] rounded-xl p-6 border border-[var(--color-border)] w-1/2">
               <p className="text-[10px] text-gray-400 tracking-widest uppercase">Pending Requests</p>
               <p className="text-3xl font-mono text-[var(--color-accent)] tracking-widest mt-2">{pendingRequests.length}</p>
            </div>
         </div>
         ...
         <div className="bg-[var(--color-card)] rounded-xl border border-[var(--color-border)] shadow-lg overflow-hidden">
             <div className="p-4 md:p-6 border-b border-[var(--color-border)] bg-[var(--color-background)]/50">
               <p className="text-xs font-bold tracking-widest uppercase text-gray-400 flex items-center gap-2">
                 <svg className="w-4 h-4 text-orange-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12" /></svg>
             ...
             <table className="w-full text-left text-sm text-gray-300 hidden md:table">
                  ...
                  <tbody className="divide-y divide-[var(--color-border)] block md:table-row-group">
                     {pendingRequests.length === 0 ? (
                       <tr><td colSpan={5} className="px-6 py-8 text-center text-gray-500">Queue is clear.</td></tr>
                     ) : (
                       pendingRequests.map((r: any) => <RequestRow key={r.id} request={r} />)
                     )}
                  </tbody>
             </table>
             ...
             <div className="md:hidden divide-y divide-[var(--color-border)]">
                 ...
                 {pendingRequests.map((r: any) => (
                    <div key={r.id} className="p-4 bg-[var(--color-card)] hover:bg-[var(--color-input)]/20">
                       <RequestRow mobile request={r} />
                    </div>
                 ))}
             </div>
         </div>
