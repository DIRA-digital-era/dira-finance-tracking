...
export default function RequestRow({ request, mobile = false, isSuperAdmin = false }: { request: any, mobile?: boolean, isSuperAdmin?: boolean }) {
       ...
       <div className="flex flex-col gap-3">
          <div className="flex justify-between items-start">
             <div>
                ...
                <p className="text-[10px] text-slate-500">{request.user_role}</p>
             </div>
          ...
          {isPending ? (
            <div className="flex gap-2">
               <button disabled={loading} onClick={() => handleResolve('APPROVED')} className="flex-1 py-2 rounded bg-[var(--color-success)]/20 text-[var(--color-success)] hover:bg-[var(--color-success)] hover:text-[var(--color-primary-foreground)] transition flex items-center justify-center gap-1 font-bold text-[10px] tracking-widest disabled:opacity-50">
                  <CheckCircle className="w-3 h-3" /> APPROVE
               </button>
               <button disabled={loading} onClick={() => handleResolve('DENIED')} className="flex-1 py-2 rounded bg-[var(--color-danger)]/20 text-[var(--color-danger)] hover:bg-[var(--color-danger)] hover:text-[var(--color-primary-foreground)] transition flex items-center justify-center gap-1 font-bold text-[10px] tracking-widest disabled:opacity-50">
                  <XCircle className="w-3 h-3" /> DENY
               </button>
            </div>
          ) : isSuperAdmin && (
            <button disabled={loading} onClick={handleOverturn} className="w-full py-2 rounded bg-[var(--color-accent)]/20 text-[var(--color-accent)] hover:bg-[var(--color-accent)] hover:text-[var(--color-primary-foreground)] transition flex items-center justify-center gap-1 font-bold text-[10px] tracking-widest disabled:opacity-50">
  ...
  return (
    <tr className="hover:bg-[var(--color-input)] transition-colors">
       <td className="px-6 py-4">
          <div className="flex items-center gap-3">
             <div className="w-8 h-8 rounded bg-[var(--color-input)] flex items-center justify-center font-bold text-slate-600">
                {request.user_name.substring(0,2).toUpperCase()}
             </div>
             <div>
                <p className="font-medium text-[var(--color-foreground)]">{request.user_name}</p>
                <p className="text-[10px] text-slate-600">{request.user_role}</p>
             </div>
          </div>
       </td>
       ...
       <td className="px-6 py-4">
          {isPending ? (
            <div className="flex gap-2 flex-wrap">
               <button disabled={loading} onClick={() => handleResolve('APPROVED')} className="px-3 py-1.5 rounded bg-[var(--color-success)]/10 text-[var(--color-success)] border border-[var(--color-success)]/20 hover:bg-[var(--color-success)] hover:text-[var(--color-primary-foreground)] transition flex items-center gap-1 font-bold text-[10px] tracking-widest disabled:opacity-50">
                  <CheckCircle className="w-3 h-3" /> APPROVE
               </button>
               <button disabled={loading} onClick={() => handleResolve('DENIED')} className="px-3 py-1.5 rounded bg-[var(--color-danger)]/10 text-[var(--color-danger)] border border-[var(--color-danger)]/20 hover:bg-[var(--color-danger)] hover:text-[var(--color-primary-foreground)] transition flex items-center gap-1 font-bold text-[10px] tracking-widest disabled:opacity-50">
                  <XCircle className="w-3 h-3" /> DENY
               </button>
            </div>
          ) : (
            <div className="flex flex-col gap-2 items-start">
              ...
              {isSuperAdmin && (
                <button disabled={loading} onClick={handleOverturn} className="px-2 py-1 rounded bg-[var(--color-accent)]/10 text-[var(--color-accent)] border border-[var(--color-accent)]/20 hover:bg-[var(--color-accent)] hover:text-[var(--color-primary-foreground)] transition flex items-center gap-1 font-bold text-[9px] tracking-widest disabled:opacity-50">
