        ...
        {tab === 'ACTIVE' ? (
          <>
            ...
            {(u.role !== 'ADMIN' && u.role !== 'SUPER_ADMIN') || currentRole === 'SUPER_ADMIN' ? (
              <>
                <button onClick={() => handleToggleStatus(u.id, u.status)} className="px-3 py-1 rounded bg-[var(--color-input)] hover:text-[var(--color-foreground)] transition text-xs">
                  {u.status === 'ACTIVE' ? 'Suspend' : 'Reactivate'}
                </button>
                <button onClick={() => handleDelete(u.id)} className="px-3 py-1 rounded bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-[var(--color-primary-foreground)] transition text-xs">
                  Delete
                </button>
              </>
            ) : (
              <span className="text-[9px] text-slate-600 italic">L3 required</span>
            )}
          </>
        ) : (
        ...
        <div className="bg-[var(--color-card)] rounded-xl border border-[var(--color-border)] shadow-lg overflow-hidden mt-6">
             ...
             <div className="p-4 border-b border-[var(--color-border)] bg-[var(--color-background)]/50 flex flex-col gap-3">
                 <div className="flex flex-wrap justify-between items-center gap-3">
                     <div className="flex bg-[var(--color-input)] p-1 rounded-lg">
                         <button onClick={() => setTab('ACTIVE')} className={`px-4 py-1.5 rounded-md text-xs font-bold tracking-widest transition whitespace-nowrap ${tab === 'ACTIVE' ? 'bg-[var(--color-primary)] text-[var(--color-primary-foreground)]' : 'text-slate-600 hover:text-[var(--color-foreground)]'}`}>CURRENT STAFF</button>
                         <button onClick={() => setTab('PAST')} className={`px-4 py-1.5 rounded-md text-xs font-bold tracking-widest transition whitespace-nowrap ${tab === 'PAST' ? 'bg-[var(--color-primary)] text-[var(--color-primary-foreground)]' : 'text-slate-600 hover:text-[var(--color-foreground)]'}`}>PAST EMPLOYEES</button>
                     </div>
                     <ViewToggle />
                 </div>
                 <div className="flex gap-2">
                     <select value={filterRole} onChange={e => setFilterRole(e.target.value)} className="bg-[var(--color-input)] border border-[var(--color-border)] text-[10px] text-[var(--color-foreground)] px-3 py-2 rounded focus:outline-none uppercase">
                     ...
                     <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Filter by name, email or title..." className="flex-1 bg-[var(--color-input)] border border-[var(--color-border)] px-3 py-2 text-xs text-[var(--color-foreground)] rounded focus:outline-none" />
                 </div>
             </div>
             ...
             {view === 'cards' ? (
               <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                 {filtered.length === 0 && <p className="col-span-full text-center text-slate-700 py-8 text-sm">No personnel match criteria.</p>}
                 {filtered.map(u => (
                   <div key={u.id} className={`bg-[var(--color-input)]/40 border border-[var(--color-border)] rounded-xl p-4 hover:border-[var(--color-primary)]/40 transition-colors flex flex-col gap-3 ${u.status === 'SUSPENDED' ? 'opacity-60' : ''}`}>
                     <div className="flex items-center gap-3">
                       <div className="w-10 h-10 shrink-0 rounded bg-[var(--color-input)] border border-[var(--color-border)] flex items-center justify-center font-bold text-slate-600">
                         {u.name.substring(0,2).toUpperCase()}
                       </div>
                       <div className="min-w-0">
                         <p className="font-bold text-[var(--color-foreground)] truncate">{u.name}</p>
                         <p className="text-[10px] text-[var(--color-primary)] font-mono truncate">{u.email}</p>
                       </div>
                     </div>
                     <div className="flex flex-wrap gap-1">
                       <span className="text-[9px] bg-[var(--color-input)] border border-[var(--color-border)] px-2 py-0.5 rounded text-slate-600 uppercase">{u.role}</span>
                     ...
                     <div className="flex items-center justify-between mt-auto pt-2 border-t border-[var(--color-border)]">
                       <span className={`flex items-center gap-1 text-[10px] font-bold ${u.status === 'ACTIVE' ? 'text-green-500' : u.status === 'DELETED' ? 'text-slate-600' : 'text-red-500'}`}>
                         <span className={`w-2 h-2 rounded-full ${u.status === 'ACTIVE' ? 'bg-green-500' : u.status === 'DELETED' ? 'bg-slate-500' : 'bg-red-500'}`}></span>
                         {u.status}
                       </span>
             ...
             <div className="overflow-x-auto">
                 <table className="w-full text-left text-sm text-[var(--color-foreground)]">
                      ...
                      <tbody className="divide-y divide-[var(--color-border)]">
                         {filtered.length === 0 ? (
                           <tr><td colSpan={5} className="px-6 py-8 text-center text-slate-700">No personnel match criteria.</td></tr>
                         ) : (
                           filtered.map((u) => (
                              <tr key={u.id} className={`hover:bg-[var(--color-input)]/20 transition-colors ${u.status === 'SUSPENDED' ? 'opacity-60' : ''}`}>
                                 <td className="px-4 py-4 min-w-[180px]">
                                    <div className="flex items-center gap-3">
                                       <div className="w-10 h-10 rounded shrink-0 bg-[var(--color-input)] border border-[var(--color-border)] flex items-center justify-center font-bold text-slate-600">
                                          {u.name.substring(0,2).toUpperCase()}
                                       </div>
                                 ...
                                 <td className="px-6 py-4 hidden lg:table-cell">
                                    <span className="bg-[var(--color-input)] border border-[var(--color-border)] px-2 py-0.5 rounded text-[10px] uppercase tracking-wider text-slate-600 block w-fit mb-1">{u.role}</span>
                                    <div className="flex flex-wrap gap-1">
                                       {(u.titles || []).map((t: string, i: number) => (
                                          <span key={i} className="text-[9px] bg-[var(--color-accent)]/10 text-[var(--color-accent)] px-2 py-0.5 rounded border border-[var(--color-accent)]/20">{t}</span>
                                       ))}
                                 ...
                                 <td className="px-6 py-4">
                                    <span className={`flex items-center gap-2 font-bold tracking-widest text-[10px] ${u.status === 'ACTIVE' ? 'text-green-500' : u.status === 'DELETED' ? 'text-slate-600' : 'text-red-500'}`}>
                                       <span className={`w-2 h-2 rounded-full shrink-0 ${u.status === 'ACTIVE' ? 'bg-green-500' : u.status === 'DELETED' ? 'bg-slate-500' : 'bg-red-500'}`}></span>
                                       {u.status}
                                    </span>
                                 </td>
