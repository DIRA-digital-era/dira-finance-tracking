...
export default function EditUserForm({ user, currentRole, jobTitles }: { user: any, currentRole: string, jobTitles: any[] }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
       ...
       <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-xl w-full max-w-lg overflow-hidden relative shadow-2xl text-left z-10">
           ...
           <div className="p-6 border-b border-[var(--color-border)] flex justify-between items-center bg-[var(--color-input)]">
              <div>
                 ...
                 <p className="text-[10px] text-slate-700 uppercase mt-1">Modify profile and clearance</p>
              </div>
              <button type="button" onClick={() => setOpen(false)} className="text-slate-600 hover:text-[var(--color-foreground)] transition relative z-20">
           ...
           <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              <div>
                 ...
                 <div className="relative">
                    ...
                    <input name="name" defaultValue={user.name} required type="text" className="w-full bg-[var(--color-input)] border border-[var(--color-border)] rounded py-3 pl-10 pr-3 text-[var(--color-foreground)] focus:outline-none focus:border-[var(--color-primary)]" />
                 </div>
              </div>
              ...
              <div>
                 ...
                 <div className="relative">
                    ...
                    <select name="role" defaultValue={user.role} required className="w-full bg-[var(--color-input)] border border-[var(--color-border)] rounded py-3 pl-10 pr-3 text-[var(--color-foreground)] focus:outline-none focus:border-[var(--color-primary)] appearance-none">
              ...
              <div>
                 <label className="block text-xs font-bold text-slate-700 mb-2 tracking-wider uppercase">Professional Titles</label>
